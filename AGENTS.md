...
- `herdr` is not in nixpkgs and is deliberately not built by this flake: its upstream flake compiles a pinned Rust toolchain from source, which takes hours and once filled the root disk. It is the native release in `~/.local/bin`, kept current by the `herdr-update` alias (detached run with live session handoff). Do not add it back as a Nix package or vendor it.
