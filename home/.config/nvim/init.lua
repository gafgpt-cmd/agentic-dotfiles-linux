...
local lazy_lock = vim.json.decode(table.concat(vim.fn.readfile(vim.fn.stdpath 'config' .. '/lazy-lock.json'), '\n'))
local lazy_commit = assert(lazy_lock['lazy.nvim'].commit, 'lazy-lock.json does not pin lazy.nvim')
assert(#lazy_commit == 40 and lazy_commit:match '^%x+$', 'lazy.nvim lock is not a full Git commit')

local function run_git(args)
  local output = vim.fn.system(args)
  if vim.v.shell_error ~= 0 then
    error(table.concat(args, ' ') .. ' failed:\n' .. output)
  end
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>S', builtin.live_grep, { desc = '[S]earch project text' })
